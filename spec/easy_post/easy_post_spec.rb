  ...
  describe '#make_client' do
    it 'respects http_config' do
      ...
      if ruby_version >= Gem::Version.new('2.5.0')
        ...
        expect(client.ssl_version).to eq :TLSv1_2 # rubocop:disable Naming/VariableNumber
      end
    end
  end
    ...
    it 'stores the http_body on the error' do
      expect { described_class.make_request('get', '/health/boom', nil) }.to raise_error(EasyPost::Error) { |error|
        expect(error.http_body).to eq(
          '{"error":{"code":"INTERNAL_SERVER_ERROR","message":"We\'re sorry, something went wrong. If the problem '\
          'persists please contact Support.","errors":[]}}',
        )
      }
    end
